...
int main(int ac, char **av)
{
	...
	while (length < wavfile->datalen)
	{
		...
		for (size_t i = 0; i < len; i += wavfile->header.channels)
		{
			...
			low_out.int32[1] = dsp_allpassLPF(LPF.C, low_out.int32[1], sample.int32[1], &LPF.z1);
			high_out.int32[1] = dsp_allpassHPF(HPF.C, high_out.int32[1], sample.int32[1], &HPF.z1);
