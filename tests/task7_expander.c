...
int main(int ac, char **av)
{
	...
	t_expander *expander = dsp_newexpander(threshold, ratio, attack_time, release_time, wavfile->header.sample_rate);
	...
	while (length < wavfile->datalen)
	{
		...
		for (size_t i = 0; i < len; i += wavfile->header.channels)
		{
			...
			wavfile->buffer[i + 1] = dsp_expander(ring, expander, wavfile->header.sample_rate, LOOKAHEAD_SIZE, WINDOW_SIZE);
