...
int main(int ac, char **av)
{
	...
	t_limiter *limiter = dsp_newlimiter(threshold, attack_time, release_time, wavfile->header.sample_rate);
	...
	while (length < wavfile->datalen)
	{
		...
		for (size_t i = 0; i < len; i += wavfile->header.channels)
		{
			...
			wavfile->buffer[i + 1] = dsp_limiter(ring, limiter, wavfile->header.sample_rate, LOOKAHEAD_SIZE, WINDOW_SIZE);
